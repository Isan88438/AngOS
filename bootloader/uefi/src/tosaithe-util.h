...
public:
    ...
    void extend_or_move(UINTN num_pages)
    {
        if (!extend_nx(num_pages)) {
            UINTN old_pages = page_count();
            EFI_PHYSICAL_ADDRESS old_address = get_ptr();
            UINTN new_total_pages = old_pages + num_pages;
            ...
            EBS->FreePages(old_address, old_pages);
            rezone(new_address, new_total_pages);
        }
    }
...
public:
    ...
    EFI_FILE_PROTOCOL *release()
    {
        EFI_FILE_PROTOCOL *result = handle_;
        handle_ = nullptr;
        return result;
    }
