...
class ang_vector
{
    ...
    void grow()
    {
        ...
        T *new_data =
            (T *)alloc_pool(sizeof(T) * new_capacity);

        if (new_data == nullptr) {
            con_write(L"ERROR: bootloader out of memory.\r\n");
            for (;;) {
                asm volatile ("hlt");
            }
        }
...
public:
    ...
    ang_vector(const ang_vector &) = delete;
    ang_vector &operator=(const ang_vector &) = delete;
...
public:
    ...
    // On allocation failure the bootloader halts.
...
// Allocation failure halts the bootloader.
...
static void *find_config_table(const EFI_GUID &table_id)
{
    ...
    for (UINTN i = 0; i < EST->NumberOfTableEntries; i++) {
        if (__builtin_memcmp(&tables[i].VendorGuid, &table_id, sizeof(EFI_GUID)) == 0) {
...
// Out-of-memory halts the bootloader.
        ...
        uintptr_t ramdisk, uint64_t ramdisk_size)
{
    ...
    for (uint16_t i = 0; i < elf_ph_ent_num; /* increment in body */) {
        ...
        if (phdr.p_type == PT_LOAD) {
            ...
            if (phdr.p_offset < first_chunk) {
                ...
                __builtin_memcpy((void *)(kernel_alloc.get_ptr() + addr_offs), ((char *)elf_hdr) + phdr.p_offset, copy_size);
    ...
    ang_vector<tsbp_kernel_mapping> tsbp_kernel_map;
    for (uint16_t i = 0; i < elf_ph_ent_num; ++i) {
        uintptr_t ph_addr = i * elf_ph_ent_size + elf_ph_off + elf_header_alloc.get_ptr();
        Elf64_Phdr phdr;
        __builtin_memcpy(&phdr, (void *)ph_addr, sizeof(phdr));
        if (phdr.p_type == PT_LOAD) {
            ...
            if (ts_entry_header == nullptr && phdr.p_memsz >= sizeof(tosaithe_entry_header)) {
                ...
                if (__builtin_memcmp(potential_hdr, "TSBP", 4) == 0) {
    ...
    for (bss_area bss : bss_areas) {
        __builtin_memset((void *)(kernel_alloc.get_ptr() + bss.begin_offset - lowest_vaddr), 0, bss.size);
    }
    ...
    EFI_MEMORY_DESCRIPTOR * efi_memmap_ptr {
    ...
    sort_efi_memmap(efi_memmap_ptr, memMapSize, memMapDescrSize);
    compact_efi_memmap(efi_memmap_ptr, memMapSize, memMapDescrSize);
    ...
    struct page_pool_record
{
    EFI_PHYSICAL_ADDRESS address;
    UINTN pages;
    page_pool_record *next;
};

page_pool_record *page_pool_records = nullptr;
    ...
    auto take_page = [&]() -> void * {
        ...
        if (alloc_pool_next == (void *)ap_end) {
            if (!page_tables_alloc_pool.extend_nx(PAGE_POOL_ALLOC_SIZE)) {
                page_pool_record *record =
                    (page_pool_record *)alloc_pool(
                        sizeof(page_pool_record)
                    );
                ...
                record->address =
                    page_tables_alloc_pool.get_ptr();

                record->pages =
                    page_tables_alloc_pool.page_count();

                record->next =
                    page_pool_records;

                page_pool_records = record;

                // Detach the old allocation so the RAII object
                // does not free the pages.
                page_tables_alloc_pool.rezone(0, 0);

                page_tables_alloc_pool.allocate(PAGE_POOL_ALLOC_SIZE);

                alloc_pool_next =
                    (void *)page_tables_alloc_pool.get_ptr();
            }
        }

        void *r = alloc_pool_next;
        alloc_pool_next = (void *)((uintptr_t)alloc_pool_next + PAGE4KB);
        __builtin_memset(r, 0, PAGE4KB);
        return r;
    };
    ...
    EFI_MEMORY_DESCRIPTOR *mmdesc = efi_memmap_ptr;
    ...
    do {
        ...
        if (mmdesc_phys_beg != 0 && mmdesc == efi_memmap_ptr) {
    ...
    if (fb_size != 0) {
        ...
        // total size.
        EFI_MEMORY_DESCRIPTOR *fb_desc = efi_memmap_find(fb_addr, efi_memmap_ptr,
    ...
    free_pool(efi_memmap_ptr); // free existing map before querying new one
    efi_memmap_ptr = get_efi_memmap(
        memMapSize,
        memMapKey,
        memMapDescrSize,
        memMapDescrVersion
    ...
    auto *efi_mem_iter = efi_memmap_ptr;
    auto *efi_mem_end = (EFI_MEMORY_DESCRIPTOR *)((char *)efi_memmap_ptr + memMapSize);
    ...
    loader_data.efi_memmap = efi_memmap_ptr;
    ...
    if (EBS->ExitBootServices(ImageHandle, memMapKey) == EFI_INVALID_PARAMETER) {
        ...
        status = EBS->GetMemoryMap(&memMapSize, efi_memmap_ptr, &memMapKey, &memMapDescrSize, &memMapDescrVersion);
        if (EFI_ERROR(status)) {
    ...
    lgdt64_ptr_struct gdt_desc { uint16_t(sizeof(gdt_array) - 1), gdt_array };
        ...
        asm volatile (
                ...
                :
                : "m"(gdt_desc), "r"(kern_stack_top), "r"(kern_entry),
